...
{	
	/***实际数据***/
    F_CopyStr(&Uart1TXDBuf,&ChipDispBuf,TOTAL_COM);
    Uart1TXDBuf[12] = g_u8VocDispState;
    Uart1TXDBuf[13] = g_u8PM25DispState;
    if(g_bHalfDispEn)
    {   Uart1TXDBuf[14] |= BIT0;}
    /***校验数据***/
...
{    
	...
	{	
        if(Uart1RXDBuf[RXD1_BUF_LENGTH-1] == CheckSum(Uart1RXDBuf,RXD1_BUF_LENGTH-1))	
        {
            ...
            {
                g_u8DispVersion = Uart1RXDBuf[0];
                g_u8KeyData = Uart1RXDBuf[1];
                g_u8SenseState = Uart1RXDBuf[2];
                g_u8RecOKTime = 0;
                return;
            }
        }
