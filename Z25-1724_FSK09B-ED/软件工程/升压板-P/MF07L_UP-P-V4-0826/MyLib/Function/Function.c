...
{
	u8 BufEn = 0;
	if(Out.UpDnAngleCol.AngleEn && (ControlDisConnect_Cnt < DIS_WORK_TIME)&& (ControlDisConnect2_Cnt < DIS_WORK_TIME))
	{
...
{
	u8 BufEn = 0;
	if(Out.LeftRightAngleCol.AngleEn && (ControlDisConnect_Cnt < DIS_WORK_TIME)&& (ControlDisConnect2_Cnt < DIS_WORK_TIME))
	{
	...
	{	
		if(Out.LeftRightAngleCol.SetCol == M_MID_NEED)
		{
			F_RL_StepMotorSwing(2048,2048,c_RL_SMSpeedSwing,0,0);
		}
		else
			F_RL_StepMotorStop();
