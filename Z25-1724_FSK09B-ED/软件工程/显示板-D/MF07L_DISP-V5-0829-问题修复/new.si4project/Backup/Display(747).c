...
#define	DS_CUP_LED1()		{g_DispSegBuf[KEY_DISP] |= BIT0;}
#define	DS_DIE_LED2()		{g_DispSegBuf[KEY_DISP] |= BIT1;}
#define	DS_WARM_LED3()		{g_DispSegBuf[KEY_DISP] |= BIT2;}
#define	DS_TEMP_LED4()		{g_DispSegBuf[KEY_DISP] |= BIT3;}
#define	DS_TIME_LED5()		{g_DispSegBuf[KEY_DISP] |= BIT4;}
#define	DS_DEC_LED6()		{g_DispSegBuf[KEY_DISP] |= BIT5;}
#define	DS_INC_LED7()		{g_DispSegBuf[KEY_DISP] |= BIT6;}
#define	DS_PAUSE_LED8()		{g_DispSegBuf[KEY_DISP] |= BIT7;}
#define	DS_POW_LED9()		{g_DispSegBuf[KEY_DISP] |= BIT8;}
#define	DS_LOCK_LED10()		{g_DispSegBuf[KEY_DISP] |= BIT9;}


#define	DS_CUP_LED1_OFF()	{g_DispSegBuf[KEY_DISP] &= ~BIT0;}
#define	DS_DIE_LED2_OFF()	{g_DispSegBuf[KEY_DISP] &= ~BIT1;}
#define	DS_WARM_LED3_OFF()	{g_DispSegBuf[KEY_DISP] &= ~BIT2;}
#define	DS_TEMP_LED4_OFF()	{g_DispSegBuf[KEY_DISP] &= ~BIT3;}
#define	DS_TIME_LED5_OFF()	{g_DispSegBuf[KEY_DISP] &= ~BIT4;}
#define	DS_DEC_LED6_OFF()	{g_DispSegBuf[KEY_DISP] &= ~BIT5;}
#define	DS_INC_LED7_OFF()	{g_DispSegBuf[KEY_DISP] &= ~BIT6;}
#define	DS_PAUSE_LED8_OFF()	{g_DispSegBuf[KEY_DISP] &= ~BIT7;}
#define	DS_POW_LED9_OFF()	{g_DispSegBuf[KEY_DISP] &= ~BIT8;}
#define	DS_LOCK_LED10_OFF()	{g_DispSegBuf[KEY_DISP] &= ~BIT9;}

#define	DS_SIGN_C()			{g_DispSegBuf[DIG2] |= BIT14 + BIT15;} 
#define	DS_SIGN_H()			{g_DispSegBuf[DIG1] |= BIT14 + BIT15;} 

#define	DS_SIGN_C_OFF()		{g_DispSegBuf[DIG2] &= ~(BIT14 + BIT15);}
#define	DS_SIGN_H_OFF()		{g_DispSegBuf[DIG1] &= ~(BIT14 + BIT15);}
...
void Display_Value_Set(void)
{
	...
	case SysMX_FstPOW_ON:
		...
		SetStr((uchar *)&g_DispSegBuf, 0xff, sizeof(g_DispSegBuf));
		g_DispSegBuf[DIG1] = SEG_NUM[0]; 
		g_DispSegBuf[DIG2] = SEG_NUM[0];
		break;
	...
	{
		g_DispSegBuf[i] |= DispComTab[i];
		//要加上另外驱动独立的5个COM
		if(g_DispSegBuf[KEY_DISP] & BIT0)
			g_DispSegBuf[i] |= BIT0; 
		if(g_DispSegBuf[KEY_DISP] & BIT1)
			g_DispSegBuf[i] |= BIT1; 
		if(g_DispSegBuf[KEY_DISP] & BIT2)
			g_DispSegBuf[i] |= BIT2; 
		if(g_DispSegBuf[KEY_DISP] & BIT3)
			g_DispSegBuf[i] |= BIT3; 
		if(g_DispSegBuf[KEY_DISP] & BIT4)
			g_DispSegBuf[i] |= BIT4; 
	}

	
	
	DispLedBufSet(g_DispSegBuf);
}
