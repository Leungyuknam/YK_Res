...
void IncDecU32Fuc(Col_Enum Col,SelectSet_Enum Object,u16 StepVal,u32 MaxVal,u32 Minval)
{
		...
		if((COOK_IDLE == Sys.MState) || (COOK_RUN_PAUSE == Sys.MState))
		{
			...
			{	
				if(Sys.Fg.CooktimeEn)
				{
					...
					u32Buf = MenuBookTimeMinTab[Sys.mMenu];
				IncDecU32Fuc(CODE_INC,SET_BOOK_TIME,STEP_BT_S,MenuBookTimeMaxTab[Sys.mMenu],u32Buf);
			...
			else if(DISP_SET_COOK_TIME == GetDispType())
			{
				if(Sys.Fg.BooktimeEn)
				{
					...
					u32Buf = MenuCookTimeMaxTab[Sys.mMenu];
				IncDecU32Fuc(CODE_INC,SET_COOK_TIME,STEP_CT_S,u32Buf,MenuCookTimeMinTab[Sys.mMenu]);
				F_DispTaskChange(DISP_SET_COOK_TIME);
				u8BuzzEn = TRUE;
			}
			else if(SetEn)
		...
		if((COOK_IDLE == Sys.MState) || (COOK_RUN_PAUSE == Sys.MState))
		{
			...
			{	
				if(Sys.Fg.CooktimeEn)
				{
					...
					u32Buf = MenuBookTimeMinTab[Sys.mMenu];
				IncDecU32Fuc(CODE_DEC,SET_BOOK_TIME,STEP_BT_S,MenuBookTimeMaxTab[Sys.mMenu],u32Buf);
				F_DispTaskChange(DISP_SET_BOOK_TIME);
				u8BuzzEn = TRUE;
			}
			else if(DISP_SET_COOK_TIME == GetDispType())
			{
				...
				IncDecU32Fuc(CODE_DEC,SET_COOK_TIME,STEP_CT_S,u32Buf,MenuCookTimeMinTab[Sys.mMenu]);
				F_DispTaskChange(DISP_SET_COOK_TIME);
				u8BuzzEn = TRUE;
			}
			else if(SetEn)
...
void KeyCooktime(void)
{
	...
	u32 u32Buf;
	FactoryKeyNumFuc(2);
	if((HUOGUO == Sys.mMenu) || (ZHENGZHU == Sys.mMenu)) 
		u8MenuCookEn = TRUE;
	if(GetDispType() == DISP_SET_BOOK_TIME) 
	{
		Sys.Fg.BooktimeEn = TRUE;
...
void KeyMenuSelect(void)
{
	...
	{
		if(++sHeat.mDispSelection > DISP_RECOVER)
			sHeat.mDispSelection = DISP_HEAT_NON;
		BUZZ();
	}
	#endif
}
