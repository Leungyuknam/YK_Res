...
u8 code TempGoalTab[] ={  								
	...
	//	60, 		80, 		110,		120,		140 ,		160,		180,		200,		220,			240    温度
		TE_36,  	TE_43,  	TE_50,  	TE_57,  	TE_64,  	TE_71, 		TE_77, 		TE_83,		TE_89,			TE_95 //TE_88//TE_98 //TE_93
};
...
u8 code LeveLMaxTab[] ={  
	//	L1			L2			L3			L4			L5			L6			L7			L8			L9				L10	
	//	60, 		80, 		110,		120,		140 ,		160,		180,		200,		220,			240    温度
		HL_4,  		HL_4, 		HL_4,  		HL_4,  		HL_4,  		HL_5, 		HL_5, 		HL_5,		HL_5,			HL_5   
};

u8 code TempHeatTimeTab[] ={ 
	//	L1			L2			L3			L4			L5			L6			L7			L8			L9				L10	
		60,			70,			80,			90,			100,		110,		100,		120,		140,			150																								
};




#define	TEMP_MENU_OFF_AD		160//165
#define	TEMP_MENU_ON_AD			126

#define	TEMP_JIANKAO_OFF_AD		250
#define	TEMP_JIANKAO_ON_AD		245
	...
	if(++ sHeat.Inter10ms >= 50)  //
	{
		...
		if(HEAT_NORMAL == sHeat.State)
		{
			if(JIANKAO == Sys.mMenu)
				u8TempOFF = TEMP_JIANKAO_OFF_AD;
			else
				u8TempOFF = TEMP_MENU_OFF_AD;	//MenuBotTempOffTab[Sys.mMenu];
		...
		{
			if((sHeat.State == HEAT_PRO_COOK) && (AD_TmprPan <= TEMP_MENU_ON_AD))
				sHeat.State = HEAT_NORMAL;
		}
...
u8 DriveControDeal(u8 NowTempDat)
{
	...
	//if(sHeat.CycLoopCnt <= 1 && (Sys.CookLevel >= TEMP_L7))
	{

		//if(sHeat.CycLoopCnt == 0)
		//	u8temp0 = 20;//35;
		//else //if(sHeat.CycLoopCnt == 1)
		//	u8temp0 = 10;
		//else if(sHeat.CycLoopCnt == 2)
		//	u8temp0 = 30;
		//else
		//	u8temp0 = 5;//30;
		//if(GoalTempAD > u8temp0)		//掐首冲
		//	GoalTempAD -= u8temp0;	
	...
	switch(sHeat.HeatStep)
	{
		...
		case 1:
			...
			else
			{
				...
				*/
				...
				if(Sys.CookLevel >= TEMP_L7 ) //&& (NowTempDat < 100)
					u8temp0 = 4;
				else
					u8temp0 = 3;
				if(sHeat.HeatCycSec >= u8temp0)
		...
		case 2:
				...
				u8temp0 = TempHeatTimeTab[Sys.CookLevel];
				...
				#else
				HeatLevel = HeatControl(NowTempDat,GoalTempAD,12,0,8,3,&sHeat);  //6  //3
				#endif
...
code MenuHeat_Str MenuCookDeal[3] = { 
	{255, 1*MINT, 5*MINT,			POW_P4,	POW_P6,	4*MINT,	40*MINT,	POW_P4,	60*MINT,	POW_P3, 90*MINT,		POW_P3},		//MENGZHU 	
	{255, 1*MINT, 5*MINT,			POW_P4,	POW_P5,	2*MINT,	45*MINT,	POW_P3,	90*MINT,	POW_P2, 0xffff,			POW_P3}, 		//BAOTANG 	
	{255, 1*MINT, 5*MINT,			POW_P4,	POW_P4,	2*MINT,	30*MINT,	POW_P3,	60*MINT,	POW_P3, 180*MINT,		POW_P2},		//MANDUN		
};
...
//		if((Fryer->DecSec > closeTime)) 		
//		{
//			if(Fryer->BHeatFlag != 1)
//			{
//				if(sHeat.CycLoopCnt < 255)		//
//					sHeat.CycLoopCnt++;
//				Fryer->BHeatFlag = 1; 
//			}
//		}
