...
{
	u8 NoPot                    	: 1;        //无锅
	u8 ErrChkNoPotBefore			: 1;		//先前加热有无锅信号
	u8 RecSI_Err					: 1;		//没有收到电源板子的有效通信数据
    u8 None							: 5;		//None
} CookSign;
...
typedef struct
{
	...
	u8 NoPotSec;					//没有锅时间计时*Sec
	u16 HeatSec;					//加热时间累加，多少时间后降功率
} Fun_t;
...
#define	NONE							0
	...
	EXT_KEY_EVENT u8 xdata KeyOnCnt = 0;
	#endif
	//默认起始功率
	...
	//默认预定时间范围		
	u32 code MenuBookTimeMinTab []   = 	{BT_MIN,		BT_MIN,			BT_MIN,			BT_MIN,		BT_MIN,		BT_MIN,		BT_MIN};
...
#else
	#ifdef _TO_CUSTOM_LIMIT_
	EXT_KEY_EVENT u8 xdata KeyOnCnt;
	#endif
...
#define	KEY_LIMIT				50		//测试程序限制开关次数
