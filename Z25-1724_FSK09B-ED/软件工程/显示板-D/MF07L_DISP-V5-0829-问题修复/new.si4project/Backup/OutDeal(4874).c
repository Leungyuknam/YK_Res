...
void OutPutDeal(void)
{
	...
	if(Sys.Fg.RLOSC && !Sys.TimeOnCount)
	{
		...
		else if((Rst_Ok == Out.LeftRightAngleState.NowStep) && (Out.LeftRightAngleCol.RollCode == Out.LeftRightAngleState.RollCode))
		{
			...
			{
				Out.LeftRightFarAngleVal = C30D_R;
				Out.LeftRightNearAngleVal = C30D_L;				
			...
			{
				Out.LeftRightFarAngleVal = C80D_R;
				Out.LeftRightNearAngleVal = C80D_L;				
	...
	if(Sys.Fg.UDOSC && !Sys.TimeOnCount)
	{
		...
		else if((Rst_Ok == Out.UpDnAngleState.NowStep) && (Out.UpDnAngleCol.RollCode == Out.UpDnAngleState.RollCode))
		{
			...
			{
				Out.UpDnFarAngleVal = UPDN_UP50;
				Out.UpDnNearAngleVal = UPDN_DN50;						
			}
			else if(Sys.mAngleUpDn == ANGLE_80)
			{
				Out.UpDnFarAngleVal = UPDN_UP80;
				Out.UpDnNearAngleVal = UPDN_DN80;						
			}
