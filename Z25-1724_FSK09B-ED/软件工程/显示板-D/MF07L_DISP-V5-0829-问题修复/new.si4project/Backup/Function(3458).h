...
#define 	C_TIME_10MS			10	

#define	TEMP_CHK_CNT		30	//判断锅底温度高、或者恢复的次数
...
typedef struct
{
	...
	//u8 TempDownDat[DOWN_AD_LENGTH];		//记录下降斜率，恢复加热用
	u8 Inter10ms;
	u8 KeepPowSec;
	s8 s8TempSlope;				//温度时间斜率
	//s8 s8TempDownSlope;				//温度下降时间斜率
	u16 ShowLevel;
	...
	//Work_Enum HeatM;
	//FstState_Enum FstTemState;
	u8 HotHeatCnt;
	u8 Heat500MS;
	u8 HotHeatAD;
	u8 ColdHeatAD;
	ForeHeat_Enum mForeState;
	Disp_Enum mDispSelection;
}OVEN_Str;
...
extern OVEN_Str xdata sHeat;

extern unsigned char code  CookLevelDispPowTab[];				//功率显示


extern volatile  unsigned char X_REG Num_LxLowOn;			//连续低功率关个数
extern volatile  unsigned char X_REG Num_LxLowOff;			//连续低功率关个数

//extern volatile bit B_Zero_No;							//无过零
//extern volatile bit B_Pan_No;							//无锅

extern void DealKey_Com();
//extern volatile unsigned char X_REG DangWei;



extern void DriveMenuReset();
extern u16 DriveMenuDeal();
extern void F_FunCtrl(void);
extern void TouchKeyDatDeal();
extern void TouchDatGet(u32 u32Dat);
extern void Drive_Heat_Control(u8 nowTemp,u8 GoalTemp,u8 tempUp,u8 tempDown,u8 maxTemp,u8 minTemp,u8 openTime,u8 closeTime,OVEN_Str *Fryer);
extern void F_TimeEvent(void);
extern void  FstPowOnDeal();
extern void Set_Work();
#endif
