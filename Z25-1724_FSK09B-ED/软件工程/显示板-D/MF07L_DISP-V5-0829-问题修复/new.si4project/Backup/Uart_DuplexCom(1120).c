...
U8 MakeEnCode()
{
	Uart1TXDBuf[0] = Out.LeftRightAngleCol;
	...
	Uart1TXDBuf[5] = Out.UpDnAngleCol;
	...
	Uart1TXDBuf[10] = Out.mFanSpeed>>8;
...
{
	static unsigned char xdata RxUartCmpBuf[3] = 0;
	...
	if(l_u8ByteNum == RXD1_BUF_LENGTH)
	{
        ...
        {
			if(TRUE == CmpStrAndCopy(&RxUartCmpBuf[0],&Uart1RXDBuf[0],3))
			{
				...
				if(1 == u8DatSame_Cnt)
				{
					if(Uart1RXDBuf[0] == 0x5a)
					{
//						*((U8 *)&Sys.Load) = pUartBuf[1];
//						BuzzSt.Byte = pUartBuf[2];
					}
				}
			}
			else
