...
void F_DispTimeOFFSet(void)
{
	if(DispFg.Bri)
	{
		g_u8DispSegBuf[DIG1] |= SEG_NUM[Sys.Time/10%10]; 
		g_u8DispSegBuf[DIG2] |= SEG_NUM[Sys.Time%10]; 
	}
	DS_SIGN_H();
}
...
void DemoTest()
{
	...
	if(++g_u8PowerON_TCnt < 4*FST_POW_INIT_TIME)
	{
		SetStr((uchar *)&g_u8DispSegBuf, 0xff, sizeof(g_u8DispSegBuf));
	}
	else
 ...
 ********************************************************************************************************/
	code DispTask_t DispTask[_DISP_TOTAL_] = 
	{
		...
		{DISP_TEMP_SET,				50,			5,				&F_DispTempSet},
	};
