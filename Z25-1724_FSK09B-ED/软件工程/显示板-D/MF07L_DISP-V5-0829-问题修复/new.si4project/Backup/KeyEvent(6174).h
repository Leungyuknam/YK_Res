...
typedef struct
{
	...
	u16	EppromCookRunSec;			//用于记录烹饪时间 = CookRunSec/60
...
#define	NONE							0
	...
	u8 code MenuLevelDefTab []   = 		{POW_P10,		POW_P8,			POW_P7,		TEMP_L9,	POW_P7,		POW_P4};
	//各个菜单最大功率选择
	u8 code MenuLevelMaxTab []   = 		{POW_P10,		POW_P10,		POW_P8,		TEMP_L10,	POW_P8,		POW_P6};
	//默认时间								
	u32 code MenuCookTimeDefTab []   = 	{90*60,			30*60,			40*60,		45*60,		60*60,		90*60};
	//默认时间设定范围
	u32 code MenuCookTimeMinTab []   = 	{CT_MIN,		CT_MIN,			40*60,		CT_MIN,		60*60,		90*60};
	u32 code MenuCookTimeMaxTab []   = 	{4*3600,		2*3600,			4*3600,		4*3600,		4*3600,		6*3600};
