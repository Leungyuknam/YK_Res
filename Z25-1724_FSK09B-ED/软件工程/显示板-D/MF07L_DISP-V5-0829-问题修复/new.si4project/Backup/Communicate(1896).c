...
//		 g_Ice.WaterCurAD = (u16)((pUartBuf[1]&0x0f)<<8) + pUartBuf[0];
//		 g_Ice.EnvAD = (u16)((pUartBuf[3]&0x0f)<<8) + pUartBuf[2];
//		 pUartBuf[0] = 0;
//		 pUartBuf[1] &= 0xf0;
//		 pUartBuf[2] = 0;
//		 pUartBuf[3] &= 0xf0;
...
//			TPIN2();
			...
			if(1 == u8DatSame_Cnt)
			{
//				*((u8 *)&g_CarRfgrt.u8CompError) = pUartBuf[4];
//				if(pUartBuf[1] & 0x80)
//					g_Ice.mSwStatus = SW_CLOSE;
//				else
//					g_Ice.mSwStatus = SW_OPEN;	

//				if(pUartBuf[1] & 0x40)
//					g_Ice.mSwSecState = SW_CLOSE;
//				else
//					g_Ice.mSwSecState = SW_OPEN; 
			}
...
unsigned char MakeEnCode(unsigned char UartBuf[])
{
	...
	unsigned char Buf = 0;
//	UartBuf[0] = *((u8 *)&g_Ice.Load);
//	UartBuf[1] =*((u8 *)&g_CarRfgrt.DCFVal);
