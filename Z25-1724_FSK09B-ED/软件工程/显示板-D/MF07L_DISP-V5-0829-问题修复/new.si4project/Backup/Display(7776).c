...
static void F_DispNormal(void)
{
	...
	if((Sys.MState != COOK_NONE) && ((Flag1_Error || Flag2_Error) || !(Status2_Work & 0x20)))
	{
		...
		if(!B_500MS_BriOn)	 //故障显示 
		{
			...
			else if(B_EPanOver)   g_u16DispSegBuf[DIG3] |= DISP_2; //锅底NTC超温故障 	
			...
			else if(!(Status2_Work & 0x20))   g_u16DispSegBuf[DIG3] |= DISP_9; //锅的开关故障或者未检测到有金属锅
		}
	}
	#else 
		#message "====================故障屏蔽======================"
...
//	static u8 DisplayStepTest = 0;
	...
	//g_u16DispSegBuf[DIG1] = SEG_NUM[DisplayStepTest];
	//g_u16DispSegBuf[DIG2] = SEG_NUM[DisplayStepTest];	
	//g_u16DispSegBuf[DIG3] = SEG_NUM[DisplayStepTest];
	//g_u16DispSegBuf[DIG4] = SEG_NUM[DisplayStepTest];

	/*

	DispBuf[DIG1] = SEG_NUM[BotNTC.Aver/1000%10];
	DispBuf[DIG2] = SEG_NUM[BotNTC.Aver/100%10];	
	DispBuf[DIG3] = SEG_NUM[BotNTC.Aver/10%10];
	DispBuf[DIG4] = SEG_NUM[BotNTC.Aver%10];


	DispBuf[DIG7] = SEG_NUM[TopNTC.Aver/1000%10];
	DispBuf[DIG12] = SEG_NUM[TopNTC.Aver/100%10];	
	DispBuf[DIG11] = SEG_NUM[TopNTC.Aver/10%10];
	DispBuf[DIG10] = SEG_NUM[TopNTC.Aver%10];

	DispBuf[DIG5] = SEG_NUM[g_u8KeyBuf/16];
	DispBuf[DIG6] = SEG_NUM[g_u8KeyBuf%16];
	*/
	//g_u16DispSegBuf[DIG1] = 0x10;
	//g_u16DispSegBuf[DIG2] = 0x20;
	//g_u16DispSegBuf[DIG3] = 0x40;
	//g_u16DispSegBuf[DIG4] = 0x80;


//#define KEY_SHORTCUT_T				BIT17
//#define KEY_BOOK_T					BIT16
//#define KEY_DEC 					BIT15
//#define KEY_INC 					BIT14
//#define KEY_MENU 					BIT18
//#define KEY_START_PAUSE				BIT19



/*
	if(BIT17 == g_u32KeyBuf)
		L_K_SHORTCUT_TIME();	
	if(BIT16 == g_u32KeyBuf)
		L_K_BOOK_TIME();	

	if(BIT15 == g_u32KeyBuf)
		L_K_DEC();	
	if(BIT14 == g_u32KeyBuf)
		L_K_INC();	

	if(BIT19 == g_u32KeyBuf)
		L_K_MENU();	
	if(BIT18 == g_u32KeyBuf)
		L_K_START_PAUSE();	
...
#define DIG_MAX			4	
	...
	g_u16DispSegBuf[DIG3] = SEG_NUM[DisplayStepTest];
	g_u16DispSegBuf[DIG4] = SEG_NUM[DisplayStepTest];
	g_u16DispSegBuf[PCOM5] = 1<<DisplayStepTest;
	if(DisplayStepTest == 1)
	{
		DS_MENU1();
		
	}
	if(DisplayStepTest == 2)
	{
		DS_MENU2();
		
	}
	if(DisplayStepTest == 3)
	{
		DS_MENU3();
		
	}
	if(DisplayStepTest == 4)
	{
		DS_MENU4();
		
	}
	if(DisplayStepTest == 5)
	{
		DS_MENU5();
		
	}
	if(DisplayStepTest == 6)
	{
		DS_MENU6();
...
//	if (GetTimeTick(TIME_1S))
//	{
//		if(++DisplayStepTest>=8)
//			DisplayStepTest = 0;
//	}

//	s4 8 3 2  1 7 6 5
//	
//	g_u16DispSegBuf[2] = 1<<DisplayStepTest;
//	g_u16DispSegBuf[DIG1] = SEG_NUM[DisplayStepTest];
//	g_u16DispSegBuf[DIG2] = SEG_NUM[DisplayStepTest];
//	g_u16DispSegBuf[DIG3] = SEG_NUM[DisplayStepTest];
//	g_u16DispSegBuf[DIG4] = SEG_NUM[DisplayStepTest];
	...
	if((COOK_RUN_PAUSE == Sys.MState) && !Flag1_Error && !Flag2_Error && (Status2_Work & 0x20))
	{ 
		...
		{
			if(DISP_RECOVER == sHeat.mDispSelection)
			{
				if(sHeat.s8TempDownSlope < 0)
				{
					u8temp0 = abs(sHeat.s8TempDownSlope);
			...
			else if(DISP_HEAT_AD == sHeat.mDispSelection)
			{
				g_u16DispSegBuf[DIG1] = SEG_NUM[sHeat.HotHeatAD/16];
				g_u16DispSegBuf[DIG2] = SEG_NUM[sHeat.HotHeatAD%16];
				
				g_u16DispSegBuf[DIG3] = SEG_NUM[sHeat.ColdHeatAD/16];
				g_u16DispSegBuf[DIG4] = SEG_NUM[sHeat.ColdHeatAD%16];
				L_W_SIGN();	
			}
			else
			{
