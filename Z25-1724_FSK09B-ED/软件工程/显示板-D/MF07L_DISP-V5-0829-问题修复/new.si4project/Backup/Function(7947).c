...
void TempChangeAnalyse()
{
	...
	if(++ sHeat.Inter10ms >= 50)  //
	{
		...
		if(HEAT_NORMAL == sHeat.State)
		{
			...
			{
				if(TEMP_NON == sHeat.FstTemState)
					sHeat.FstTemState = TEM_HOT;
			...
			else
			{
				if(TEMP_NON == sHeat.FstTemState)
				{
					sHeat.FstTemState = TEM_COLD;
				}
				


				if(sHeat.s8TempSlope >= 25)
				{
					if(++sHeat.HotHeatCnt >= 6*C_SLOPE1S)
					{
						sHeat.HotHeatCnt = 0;
			...
			{
				Sys.TempHigh_Cnt = Sys.TempLow_Cnt = 0; 				
			}
		}
		else
			sHeat.mForeState = FORE_NON;	//保护不强制功率输出
		...
		for(i = 0; i < DOWN_AD_LENGTH - 1; i++)
			sHeat.TempDownDat[i] =	sHeat.TempDownDat[i+1];
		sHeat.TempDownDat[DOWN_AD_LENGTH - 1] = AD_TmprPan;
		for(i = 0; i < DOWN_AD_LENGTH; i++)
		{
			if(MinDat > sHeat.TempDownDat[i])
			{
				MinDat = sHeat.TempDownDat[i];
				MinDatNO = i;
			}
			
			if(MaxDat <= sHeat.TempDownDat[i])
			{
				MaxDat = sHeat.TempDownDat[i];
				MaxDatN0 = i;
			}
		}
		...
		if(MinDatNO < MaxDatN0)
		{
			...
			//	sHeat.s8TempSlope = (s8)(sHeat.TempDat[STORE_LENGTH - 1] - MaxDat); //温度递减趋势
			//else
				sHeat.s8TempDownSlope = (s8)(MaxDat - MinDat);	//温度递增趋势
		}
		else
		{
			//if(MinDat  < sHeat.TempDat[STORE_LENGTH - 1])
			//	sHeat.s8TempSlope = (s8)(sHeat.TempDat[STORE_LENGTH - 1] - MinDat); //温度递增趋势
			//else
				sHeat.s8TempDownSlope = (s8)( MinDat - MaxDat);  //温度递减趋势
		}
		if(sHeat.State == HEAT_PRO_COOK)
		{
			if(sHeat.s8TempDownSlope <= -7)
			{
				sHeat.State = HEAT_NORMAL;
			}
		}
