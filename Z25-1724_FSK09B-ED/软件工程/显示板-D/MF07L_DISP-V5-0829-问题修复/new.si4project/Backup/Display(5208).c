...
void FactoryDisplay_Value_Set(void)
{
	...
	switch (FactoryStep) 
	{
		...
		case 1:
			...
			else if(g_sTempAD.AD_Staut != c_AD_NORMAL)
			{
				if(B_500MS_BriOn)
				{
					g_u8DispSegBuf[DIG1] |= DISP_E; 
			...
			{
				g_u8DispSegBuf[DIG1] = SEG_NUM[Sys.EnvironTemp/10%10];
				g_u8DispSegBuf[DIG2] = SEG_NUM[Sys.EnvironTemp%10];
			}
			else
			{
...
void DemoTempAD()
{
	...
	{
//		g_u8DispSegBuf[DIG1] = 0;
//		g_u8DispSegBuf[DIG2] = SEG_NUM[Sys.TempAD/256%16];
		g_u8DispSegBuf[DIG1] =  SEG_NUM[(Sys.EnvironTemp/10)%10];	//Fuc.EnvironTemp
		g_u8DispSegBuf[DIG2] =  SEG_NUM[Sys.EnvironTemp%10]; 
		DS_SIGN_C();
	}
}
...
void Display_Value_Set(void)
{
	...
	case SysMX_Run:
		...
		{
			 if(g_sTempAD.AD_Staut == c_AD_OVERSIZE)
			 {
				g_u8DispSegBuf[DIG1] = DISP_E; 
			 	g_u8DispSegBuf[DIG2] = DISP_1;
			 }
		...
		else
		{
			...
			if(COOK_RUN_PAUSE == Sys.MState)
			{
				if(B_500MS_BriOn)
				{
					g_u8DispSegBuf[DIG1] = SEG_NUM[sHeat.DispHotStep/10%10]; 
					g_u8DispSegBuf[DIG2] = SEG_NUM[sHeat.DispHotStep%10];
					DS_SIGN_C();
					DS_SIGN_H();
				}
				else
