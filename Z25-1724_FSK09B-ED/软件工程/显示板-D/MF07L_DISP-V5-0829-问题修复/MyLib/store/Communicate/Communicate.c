...
//    	TPIN1();
         Result = DECODE_PASS;
		if(TRUE == CmpStrAndCopy(&RxUartCmpBuf[0],&pUartBuf[0],6))
		{
//			TPIN2();
			...
			if(1 == u8DatSame_Cnt)
			{
				g_u16DispSegBuf[0] = pUartBuf[0] + (u16)((pUartBuf[1]&0x07)<<8);
				g_u16DispSegBuf[1] = pUartBuf[2] + (u16)((pUartBuf[1]&0x70)<<4);
				
				g_u16DispLowSegBuf[0] = pUartBuf[3] + (u16)((pUartBuf[4]&0x07)<<8);
				g_u16DispLowSegBuf[1] = pUartBuf[5] + (u16)((pUartBuf[4]&0x70)<<4);
			}
		}
		else
			u8DatSame_Cnt = 0;
...
unsigned char MakeEnCode(unsigned char UartBuf[])
{
	...
	UartBuf[0] = u16TouchKey&0xff; 		
	UartBuf[1] = (u8)(u16TouchKey>>8); 	
	UartBuf[2] = F_CheckNumCrc8(UartBuf, 2);
    return 3;
}
...
unsigned char MakeEnCode(unsigned char UartBuf[])
{
	UartBuf[0] = (u8)(V12IN_Heat.Aver>>8); 	
	UartBuf[1] = V12IN_Heat.Aver&0xff; 		
	UartBuf[2] = *((U8 *)&PowInfo.Door);
	//新增加 2023.11.17
	UartBuf[3] = (u8)(V12IN_Micro.Aver>>8); 	//FoodNTC
	UartBuf[4] = V12IN_Micro.Aver&0xff; 		
	UartBuf[5] = *((U8 *)&PowInfo.PowPinState);
	UartBuf[6] = *((U8 *)&PowInfo.HumiMicroPow);
