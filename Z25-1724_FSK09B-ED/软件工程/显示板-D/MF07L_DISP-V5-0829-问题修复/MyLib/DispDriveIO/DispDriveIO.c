...
{
	static u8 u8DispCnt,u8ComOn_Cnt,u8DispLow_TCnt;
	u16 u16DispTemp0;
	if(++u8DispCnt >= 4)
	{
		...
		{
			u8DispCnt = 0;	//1MS
			if(++ u8DispLow_TCnt >= COM_MAX_CNT*4)
			{
				u8DispLow_TCnt = 0;
			}
			if(++ u8ComOn_Cnt >= COM_MAX_CNT)		
				u8ComOn_Cnt = 0;
			if(u8DispLow_TCnt < COM_MAX_CNT)
				u16DispTemp0 = g_DispSeg[u8ComOn_Cnt];  		//高、低亮都会显示的数据
			else
				u16DispTemp0 = g_DispSegHalf[u8ComOn_Cnt]; 
			All_COM_OFF();
...
void DispLedBufSet(u8 *u8HalfBuf, u8 *u8Buf)
{
	u8 i = 0;
	//驱动缓存赋值
	B_DispUpdating = TRUE;
