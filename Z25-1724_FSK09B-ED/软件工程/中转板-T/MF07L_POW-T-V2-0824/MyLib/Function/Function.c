...
U16 xdata t_FanSpd[] = {0, 600, 740, 880, 1020, 1160, 1300, 1440, 1600};
...
void F_UDOscEvent()
{
	u8 BufEn = 0;
	u8 RstMidEn = 0;
	if(Out.UpDnAngleCol.AngleEn && (ControlDisConnect_Cnt < DIS_WORK_TIME))
	{
	...
	{	
		if(Out.UpDnAngleCol.SetCol == M_MID_NEED)
		{
			RstMidEn = 0;
			F_StepMotorSwing(UPDN_DN180/2,UPDN_DN180/2,c_SMSpeedSwing,0,0);
		}
		else
			F_StepMotorStop();
...
{
	u8 BufEn = 0;
	if(Out.LeftRightAngleCol.AngleEn && (ControlDisConnect_Cnt < DIS_WORK_TIME))
	{
...
void PWM_Set(u16 Frq)
{
	...
	if(Frq)
	{
		...
		PWM0_MAP = 0x23;					//P23端口 //PWM0通道映射P11口
...
void F_FanSpeedEvent()
{
	...
	//PWM_Set(316);//test
	//#if 1
	if(ControlDisConnect_Cnt >= DIS_WORK_TIME)
	{
		PWM_Set(0);
		LastDat = 0;
	}
	else if(Out.FanSpeed != LastDat)
	{
		LastDat = Out.FanSpeed;
//		if(1 == Out.FanSpeed)
//			PWM_Set(316);
//		else if(2 == Out.FanSpeed)
//			PWM_Set(371);
//		else if(3 == Out.FanSpeed)
//			PWM_Set(426);
//		else if(4 == Out.FanSpeed)
//			PWM_Set(481);
//		else if(5 == Out.FanSpeed)
//			PWM_Set(536);
//		else if(6 == Out.FanSpeed)
//			PWM_Set(591);
//		else if(7 == Out.FanSpeed)
//			PWM_Set(650);
		if(Out.FanSpeed < 9)
		{
			PWM_Set(t_FanSpd[Out.FanSpeed] / 4);
		}
		else
			PWM_Set(0);	
