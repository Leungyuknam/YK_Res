...
//外部变量接口
extern 	unsigned	char	data		CurrentChannelMax;		// 当前选中的触摸通道数量
...
#define		SOCAPI_CFG_OVERLOW_MAX_COUNT			    10		// 低基线更新速度设置：出现低基线后每多少轮更新一次，更新步进为噪声值/2；数值越小，更新越快
...
*备注：返回值依据 TouchKeyRestart() 的调用周期设置
...
*备注	 ：1,  调用触控库检测函数SensorKeyFlag()
		   ...
		   3,  检测是否需要立即更新基线:  大于MAX_KEY_RESET_BASELINE 个按键按下时立即更新baseline
		   4,  双键或者单键按下时， 时间大于SetOneKeyPushResetTime()结果时更新基线 
**************************************************/
unsigned long int TouchKeyScan(void)
{
	...
	if(GetIsNeedUpdateBaseline() == 0)				// 检测是否需要更新基线
	{
		Keyvalue = SensorKeyFlag();					// 传感器判断；bMultiple=1 表示检测过程中存在干扰	 //分析按键，得出标准的16 通道位                     							   
		...
		{			
			...
			{
				SetNeedUpdateBaseline(); 							// 立即更新基线，例如盖上亚克力板
			}
			else
			{					
		...
		{		
			if(KeyData != 0x0)					    //单个按键达到多长时间就更新基线 ,松手检测
			{			
				UpdateBaseLNum++; 
			}
			else	
