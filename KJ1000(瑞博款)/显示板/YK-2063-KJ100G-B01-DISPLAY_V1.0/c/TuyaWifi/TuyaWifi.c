...
s32 MCU2WIFI(unsigned char Command)
{
	...
	else if(Command == WIFI_CMD_WORK_MODE)
  {
	...
	{
		temp = g_s8TempDisData;
	}
	else if (Command == WIFI_CMD_HUMI_DATA)
	{
		temp = g_u8HumiDisData;
	}
...
//    }
    ...
    temp |= WorkStateBuf[2];
    if(temp != g_u16DispPm25Data)
    {
        WorkStateBuf[1] = g_u16DispPm25Data >> 8;
        WorkStateBuf[2] = g_u16DispPm25Data;
    ...
    if(WorkStateBuf[6] != g_s8TempDisData)
    {
        WorkStateBuf[6] = g_s8TempDisData;
        TuyaWifiTask(WIFI_CMD_TEMP_DATA);
    }
    
    if(WorkStateBuf[7] != g_u8HumiDisData)
    {
        WorkStateBuf[7] = g_u8HumiDisData;
...
{        
	...
	if(F_GetTimeTick(C_TIME_1s))
	{
		...
		else
		{
			...
			else if((g_u8MCURunTime > 30) && (!g_bWifiWorkStart))	//上电30秒后，未启动配网、测试模式、联网，则自动配网
			{
				g_bWifiWorkStart = 1;
