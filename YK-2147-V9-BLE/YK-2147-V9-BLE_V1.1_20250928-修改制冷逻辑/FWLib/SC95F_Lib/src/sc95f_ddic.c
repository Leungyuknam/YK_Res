...
#if defined(SC95F8x1x) || defined(SC95F7x1x) || defined(SC95F8x3x) || defined(SC95F7x3x)

#if defined(SC95F8x1x) || defined(SC95F7x1x)
uint8_t xdata LCDRAM[30] _at_ 0x1000;
#elif defined (SC95F8x3x) || defined (SC95F7x3x) 
...
DDIC_Pin_TypeDef:P3OutputPin:设置P3口引脚为LCD电压输出口（uint8_t作为入参，方便进行位与运算）
*出口参数:void
**************************************************/
...
DDIC_Control_COM_TypeDef:DDIC_Com:选择控制的COM口（uint8_t作为入参，方便进行位与运算）
